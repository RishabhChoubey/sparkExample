...
object StructTypeUsage extends App{
  ...
  def printSc(schema:StructType,name:String): Unit =
  {
    schema.fields.map(f=>{
      val newName= if(name=="") f.name else name+"."+f.name
      println("new name "+ newName)
      f.dataType match{
        case s:StructType=> printSc(s,newName)
        case _=> println(newName+"   "+ f.dataType)
      }
